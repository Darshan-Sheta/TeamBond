...
public class GithubApiService {
     ...
     */
    public Map<RepositoryInfo, List<String>> getFrameworksForRepositories(List<RepositoryInfo> repositories,
            String owner, String accessToken) {
        ...
        // Once main thread gets the output from the first thread then only does it go
        // for the second one
        ...
        try {
            if (!frameworkExecutor.awaitTermination(60, TimeUnit.SECONDS)) { // If this returns false then shut down
                                                                             // immediately meaning if the task not
                                                                             // completed in the given time then just
                                                                             // simply shutdown
    ...
    /**
     * Counts the number of files associated with each framework across
     * repositories.
     * Processes commits in parallel with a thread pool sized based on the number of
     * commits.
     * 
     ...
     */
    public Map<String, Integer> countFrameworkFiles(Map<RepositoryInfo, List<String>> repoToFrameworks, String owner,
            String accessToken) {
        ...
        for (Map.Entry<RepositoryInfo, List<String>> entry : repoToFrameworks.entrySet()) {
            ...
            int commitThreadPoolSize = Math.min(repo.getCommitShas().size(), 100); // Cap at 100 threads for commit
                                                                                   // processing
            ...
            for (String commitSha : repo.getCommitShas()) {
                Callable<Void> task = () -> {
                    String commitUrl = "https://api.github.com/repos/" + owner + "/" + repo.getName() + "/commits/"
                            + commitSha;
                    try {
                        ResponseEntity<JsonNode> response = restTemplate.exchange(commitUrl, HttpMethod.GET, entity,
                                JsonNode.class);
                        ...
                        for (JsonNode file : files) {
                            ...
                            for (String framework : frameworks) {
                                List<String> extensions = Mappings.FRAMEWORK_TO_FILE_EXTENSIONS.getOrDefault(framework,
                                        Collections.emptyList());
                                for (String ext : extensions) {
                                    if (filename.endsWith(ext)) {
                                        globalFrameworkToFiles.computeIfAbsent(framework, k -> new HashSet<>())
                                                .add(repoFilePath);
                                        break;
                                    }
                                }
                        ...
                        System.err.println("Error fetching commit " + commitSha + " for repo " + repo.getName() + ": "
                                + e.getMessage());
                    }
                    return null;
                };
    ...
    public List<RepositoryInfo> listAllRepositories(String username, String accessToken) {
        ...
        if (data != null && data.get("user") != null && data.get("user").get("repositories") != null) {
            JsonNode nodes = data.get("user").get("repositories").get("nodes");
    ...
    /**
     * Tool for AI Agent: Reads specific file content (e.g., package.json).
     */
    public String getFileContent(String owner, String repoName, String path, String accessToken) {
        String url = "https://api.github.com/repos/" + owner + "/" + repoName + "/contents/" + path;
        HttpHeaders headers = new HttpHeaders();
        headers.setBearerAuth(accessToken);
        headers.set("Accept", "application/vnd.github.v3.raw"); // Request raw content directly
    ...
    public List<String> getFrameworkFromRepository(RepositoryInfo repo, String owner, String accessToken) {
        String url = "https://api.github.com/repos/" + owner + "/" + repo.getName() + "/git/trees/"
                + repo.getDefaultBranch() + "?recursive=1";
        HttpHeaders headers = new HttpHeaders();
        headers.setBearerAuth(accessToken);
        HttpEntity<String> entity = new HttpEntity<>(headers);
        ...
        for (JsonNode item : tree) { // For each file ...
            if (item.get("type").asText().equals("blob")) {
                ...
                for (String configFile : configFilesToLookFor) { // Check if it matches one of the config files we are
                                                                 // looking for
        ...
        for (String configPath : configFilePaths) {
            ...
            String contentUrl = "https://api.github.com/repos/" + owner + "/" + repo.getName() + "/contents/"
                    + configPath + "?ref=" + repo.getDefaultBranch();
            ResponseEntity<JsonNode> contentResponse = restTemplate.exchange(contentUrl, HttpMethod.GET, entity,
                    JsonNode.class);
            ...
            List<Mappings.DependencyFramework> dependencyFrameworks = Mappings.CONFIG_TO_DEPENDENCY_FRAMEWORK
                    .get(configFileName);
